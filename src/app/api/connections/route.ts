...
import { db } from '@/db';
import { connections, users, sessions, activityLog, notifications } from '@/db/schema';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const results = await Promise.all(userConnections.map(async (connection) => {
      const isRequester = connection.requesterId === user.id;
      const otherUserId = isRequester ? connection.responderId : connection.requesterId;

      let otherUserData;
      if (isRequester) {
        const responderData = await db.select({
          id: users.id,
          name: users.name,
          email: users.email,
          role: users.role,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof responderId !== 'number' || isNaN(responderId) || responderId <= 0) {
      return NextResponse.json({ 
    ...
    const responder = await db.select()
      .from(users)
    ...
    if (responder[0].status !== 'active' && responder[0].status !== 'approved') {
      return NextResponse.json({ 
