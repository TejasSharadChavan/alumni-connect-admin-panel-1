...
export async function GET(request: NextRequest) {
  try {
    ...
    const topics = [
      {
        topic_id: 'topic_1',
        name: topKeywords.slice(0, 3).map(k => k.word).join(', '),
        keywords: topKeywords.slice(0, 5).map(k => k.word),
        weight: 0.25,
        post_count: recentPosts.filter(p => 
          topKeywords.slice(0, 3).some(k => p.content.toLowerCase().includes(k.word))
        ).length,
      },
      {
        topic_id: 'topic_2',
        name: topKeywords.slice(3, 6).map(k => k.word).join(', '),
        keywords: topKeywords.slice(3, 8).map(k => k.word),
        weight: 0.20,
        post_count: recentPosts.filter(p => 
          topKeywords.slice(3, 6).some(k => p.content.toLowerCase().includes(k.word))
        ).length,
      },
      {
        topic_id: 'topic_3',
        name: topKeywords.slice(6, 9).map(k => k.word).join(', '),
        keywords: topKeywords.slice(6, 11).map(k => k.word),
        weight: 0.15,
        post_count: recentPosts.filter(p => 
          topKeywords.slice(6, 9).some(k => p.content.toLowerCase().includes(k.word))
        ).length,
      },
    ];
