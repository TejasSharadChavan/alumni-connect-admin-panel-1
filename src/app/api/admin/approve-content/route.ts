  ...
  try {
    ...
    if (!type || !ALLOWED_CONTENT_TYPES.includes(type)) {
      return NextResponse.json(
        {
          error:
            "Invalid content type. Must be one of: posts, jobs, events, campaigns",
    ...
    // If it's a job, notify all students
    if (type === "jobs") {
      try {
        ...
        const studentNotifications = students.map((student) =>
          db.insert(notifications).values({
            ...
            message: `${content.company || "A company"} is hiring for ${content.title} (${content.jobType || "position"}). Check it out now!`,
            relatedId: contentId.toString(),
            isRead: false,
            createdAt: now,
          })
        ...
        console.error("Error notifying students about approved job:", error);
      }
    }
