...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !["like", "heart", "celebrate", "support", "insightful"].includes(
        reactionType
      )
    ) {
      return NextResponse.json(
        {
          error:
            "Invalid reaction type. Must be one of: like, heart, celebrate, support, insightful",
