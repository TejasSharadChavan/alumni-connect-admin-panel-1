...
export async function GET(
  ...
  try {
    ...
    const submission = {
      ...result[0],
      technologies: result[0].technologies ? JSON.parse(result[0].technologies as string) : [],
    };
...
export async function PUT(
  ...
  try {
    ...
    if (existing.submittedBy !== user.id) {
      return NextResponse.json({ error: "You can only update your own submissions" }, { status: 403 });
    }

    if (existing.status !== "pending" && existing.status !== "revision_requested") {
      return NextResponse.json({ error: "Cannot update submission with current status" }, { status: 400 });
    }

    const body = await req.json();
    const { title, description, repositoryUrl, demoUrl, documentUrl, technologies } = body;
