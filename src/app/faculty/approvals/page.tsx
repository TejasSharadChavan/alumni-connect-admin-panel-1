...
import { FileCheck, Clock, CheckCircle, XCircle, Loader2, Eye, Github, ExternalLink, FileText, Upload } from "lucide-react";
...
export default function FacultyApprovalsPage() {
  ...
  const handleReviewProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-submissions/${selectedProject.id}/review`, {
        method: "POST",
        headers: {
  ...
  const reviewedProjects = projectSubmissions.filter(p => p.status !== "pending");
    ...
    <RoleLayout role="faculty">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Review content and project submissions</p>
        </div>
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardTitle className="text-3xl">
                {reviewedContent.filter((c) => c.status === "approved").length + reviewedProjects.filter(p => p.status === "approved").length}
              </CardTitle>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardTitle className="text-3xl">
                {pendingContent.length + reviewedContent.length + projectSubmissions.length}
              </CardTitle>
            </CardHeader>
          </Card>
        </div>
            ...
            {pendingProjects.length === 0 ? (
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">No Pending Project Submissions</h3>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="space-y-2 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline">{project.submitterBranch || "N/A"}</Badge>
                            <Badge variant="secondary">Team: {project.teamName}</Badge>
                          </div>
                          <CardTitle className="text-xl">{project.title}</CardTitle>
                          <CardDescription>
                            By {project.submitterName} ({project.submitterEmail}) • {new Date(project.submittedAt).toLocaleDateString()}
                          </CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    ...
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div className="space-y-2">
                          ...
                          <CardTitle className="text-xl">{project.title}</CardTitle>
                          <CardDescription>
                            By {project.submitterName} • Reviewed {project.reviewedAt ? new Date(project.reviewedAt).toLocaleDateString() : ""}
                          </CardDescription>
                        </div>
                      </div>
                    </CardHeader>
        ...
        <Dialog open={projectDialogOpen} onOpenChange={setProjectDialogOpen}>
          <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>Review the complete project submission</DialogDescription>
            </DialogHeader>
            ...
            {selectedProject && (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <Badge className="bg-blue-500">Project</Badge>
                  <Badge variant="outline">{selectedProject.submitterBranch || "N/A"}</Badge>
                  <Badge variant="secondary">Team: {selectedProject.teamName}</Badge>
                </div>

                <div>
                  <h3 className="text-xl font-bold mb-2">{selectedProject.title}</h3>
                  <p className="text-sm text-muted-foreground">
                    Submitted by {selectedProject.submitterName} ({selectedProject.submitterEmail}) on{" "}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedProject.repositoryUrl && (
                      <Button variant="outline" size="sm" asChild>
                        <a href={selectedProject.repositoryUrl} target="_blank" rel="noopener noreferrer">
                          <Github className="h-4 w-4 mr-2" />
                          Repository
                        </a>
                      </Button>
                    ...
                    {selectedProject.documentUrl && (
                      <Button variant="outline" size="sm" asChild>
                        <a href={selectedProject.documentUrl} target="_blank" rel="noopener noreferrer">
                          <FileText className="h-4 w-4 mr-2" />
                          Documentation
                        </a>
                      </Button>
                    )}
                    {!selectedProject.repositoryUrl && !selectedProject.demoUrl && !selectedProject.documentUrl && (
          ...
          <DialogContent>
            ...
            <form onSubmit={handleReviewProject} className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
                  value={reviewAction}
                  onValueChange={(value: "approve" | "reject" | "request_revision") => setReviewAction(value)}
                  required
                >
